...
function Api(platform, platformRootDir, events) {
  ...
  this._munger = new PlatformMunger(
    platform,
    this.root,
    this._platformJson,
    this._pluginInfoProvider
  );
}
...
Api.createPlatform = function(dest, config, options, events) {
  ...
  try {
    ...
    result = creator.createProject(dest, id, name, options).then(function() {
...
Api.prototype.prepare = function(cordovaProject, options) {
  ...
  if (fs.existsSync(defaultConfigPath)) {
    this.events.emit(
      'verbose',
      'Generating config.xml from defaults for platform "' + this.platform + '"'
    );
    shell.cp('-f', defaultConfigPath, ownConfigPath);
  } else if (fs.existsSync(ownConfigPath)) {
    this.events.emit(
      'verbose',
      'Generating defaults.xml from own config.xml for platform "' + this.platform + '"'
    );
  ...
  xmlHelpers.mergeXml(
    cordovaProject.projectConfig.doc.getroot(),
  ...
  if (fs.existsSync(srcManifestPath)) {
    ...
    if (this.config) {
      ...
      var manifestIcons = icons.map(function(icon) {
        ...
        return { src: icon.src, type: 'image/png', sizes: icon.width + 'x' + icon.height };
      });
      ...
      if (fs.existsSync(startUrlPath)) {
        ...
        if (result && result.length >= 2) {
          ...
          // see if there is a preference in config.xml
...
Api.prototype.addPlugin = function(pluginInfo, installOptions) {
  ...
  if (!pluginInfo) {
    return Promise.reject(
      new Error(
        'The parameter is incorrect. The first parameter ' + 'should be valid PluginInfo instance'
      )
    );
  }
  ...
  pluginInfo
    .getFilesAndFrameworks(this.platform)
  ...
  return actions.process(this.platform, this.root).then(function() {
    if (projectFile) {
      projectFile.write();
    }
...
Api.prototype.removePlugin = function(plugin, uninstallOptions) {
  ...
  // CB-10108 platformVersion option is required for proper plugin installation
  uninstallOptions.platformVersion =
    uninstallOptions.platformVersion || this.getPlatformInfo().version;
  ...
  plugin
    ...
    .forEach(function(item) {
      actions.push(
        actions.createAction(
          self._getUninstaller(item.itemType),
          [item, plugin.dir, plugin.id, uninstallOptions, projectFile],
          self._getInstaller(item.itemType),
          [item, plugin.dir, plugin.id, uninstallOptions, projectFile]
        )
      );
...
Api.prototype._addModulesInfo = function(plugin, targetDir) {
  ...
  var modulesToInstall = plugin
    .getJsModules(this.platform)
    .filter(function(moduleToInstall) {
      return installedPaths.indexOf(moduleToInstall.file) === -1;
    })
    .map(function(moduleToInstall) {
      var moduleName =
        plugin.id + '.' + (moduleToInstall.name || moduleToInstall.src.match(/([^\/]+)\.js/)[1]);
      var obj = {
